...
import com.pramodlakmal.game2048.presenter.GamePresenterSmall
import com.pramodlakmal.game2048.repository.GameRepository
import com.pramodlakmal.game2048.repository.GameRepositorySmall
...
class GameSmallActivity : AppCompatActivity(), GameContractSmall.View {
    ...
    private var btnUndoType = false

    //    private lateinit var dialog: DialogExit
    private lateinit var btnUndo2: ImageView
    private val buttons: ArrayList<TextView> = ArrayList(9)
    private val presenter = GamePresenterSmall(this, GameRepositorySmall())
    ...
    override fun onPause() {
        if (btnRestartFinish) {
            LocalData.setGameUndoTypeSmall(true)
            presenter.restart()
        }else{
            presenter.saveData()
    ...
    override fun changeState(matrix: Array<Array<Int>>) {
        if (!LocalData.getGameUndoTypeSmall()) {
...
//        }
        ...
        movementDetector.setOnMovementListener {
            btnUndo.visibility = View.VISIBLE
            btnUndo2.visibility = View.INVISIBLE
            LocalData.setGameUndoTypeSmall(false)
            when (it) {
